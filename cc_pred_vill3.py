...
for i in xrange(len(xx)):
    ...
    #     continue
    if xx[i][label[0]] == '' or xx[i][label[0]] == '4':
        continue
    xxx = []
    yyy = []
    for k in feat:
...
# header[5] = 'Source of Lighting'
# header[6] = 'Source of Water'
# header[10] = 'Employment'
for i in [0]:
    label_id = i+1
    y = yy[:, label_id - 1:label_id]
    y = y.reshape(len(x))
    ...
    for j in xrange(len(y)):
        if y[j] != 1 and y[j] != 2 and y[j] != 3:
            print (str(j) + 'mew?')
    rus = RandomUnderSampler(random_state=42)
    x2,y  = rus.fit_sample(x, y)
    print x2.shape
    clf.fit(x2,y)
    # y_pred = clf.predict(x)
    y_pred = cross_val_predict(clf,x2,y,cv=5)
