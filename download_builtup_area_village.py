...
fcD = ee.FeatureCollection('ft:13YnFNSd-d1x6IRiirVUw8sPLZhtMRAjFTS7s1LTr','geometry')
# fcD = ee.FeatureCollection('ft:17FoEu1aJZbmr4VxHXaSf7YrKqqkebJgFmFKY6wIL','geometry')
# print(fcD.size().getInfo())
...
def getImageMin(geom,y1,m1,y2,m2):
    c1 = ee.ImageCollection('LANDSAT/LT5_L1T_TOA').select(['B7']).filterBounds(geom).filterDate(y1+'-'+m1+'-01',y2+'-'+m2+'-01').filter(ee.Filter.lt('CLOUD_COVER',10))
    c2 = ee.ImageCollection('LANDSAT/LC08/C01/T1_TOA').select(['B7']).filterBounds(geom).filterDate(y1+'-'+m1+'-01',y2+'-'+m2+'-01').filter(ee.Filter.lt('CLOUD_COVER',10))
...
def gitIm(district):
    ...
    nl = ee.ImageCollection('NOAA/VIIRS/DNB/MONTHLY_V1/VCMSLCFG').filterBounds(district.geometry()).median().select('avg_rad');
    ...
    ss = nl.reduceRegion(reducer=ee.Reducer.sum(),
    ...
    print(ss.getInfo())
    # res = res.set('s_nl',ss);
    ss = nl.reduceRegion(reducer=ee.Reducer.mean(),
                         geometry=district.geometry(),
                         scale=30,
                         maxPixels=1e9
                         ).get('constant');
    # res = res.set('m_nl',ss);
    print(ss.getInfo())
    ss = ff.reduceRegion(reducer=ee.Reducer.sum(),
                         geometry=district.geometry(),
                         scale=30,
                         maxPixels=1e9
                         ).get('constant');
    ...
    # res = res.set('s_bu',ss);
    ss = ff.reduceRegion(reducer=ee.Reducer.mean(),
                         geometry=district.geometry(),
                         scale=30,
                         maxPixels=1e9
                         ).get('constant');
...
villages_list = fcD.toList(200000).getInfo()
